$(function () {
    ...
    // 初始化 API URL 复制按钮与事件
    try {
        if (!window.__apiUrlCopyBound) {
            // 事件委托：点击复制
            $(document).on('click', '.api-url-copy', function () {
                var btn = $(this);
                var url = btn.prevAll('kbd.api-url').first().text().trim();
                if (!url) return;

                function ok() {
                    if (window.layer && layer.msg) { layer.msg('已复制'); } else { alert('已复制'); }
                }
                function fail() {
                    if (window.layer && layer.msg) { layer.msg('复制失败'); } else { alert('复制失败'); }
                }
            ...
            window.__apiUrlCopyBound = true;
        }

        // 插入复制按钮（避免重复）
        window.ensureApiUrlCopyButtons = function () {
        ...
        // 首次与延时多次确保（覆盖异步渲染）
        window.ensureApiUrlCopyButtons();
        setTimeout(window.ensureApiUrlCopyButtons, 50);
        setTimeout(window.ensureApiUrlCopyButtons, 200);
    } catch (e) {}
...
function doCreateApi() {
    // 提交前同步编辑器内容
    if (typeof syncMarkdownEditors === 'function') {
        syncMarkdownEditors();
    }
...
function updateThemeActive(theme) {
    var ids = ['theme-blue-btn', 'theme-orange-btn', 'theme-dark-btn'];
    ids.forEach(function (id) {
        var el = document.getElementById(id);
        if (!el) return;
        el.className = el.className.replace(/\bactive\b/g, '').trim();
    });
    var currentId = theme + '-btn';
    var el2 = document.getElementById(currentId);
    if (el2 && !/\bactive\b/.test(el2.className)) {
...
function applyTheme(theme) {
    ...
    var classes = ['theme-blue', 'theme-orange', 'theme-dark'];
...
function initThemeSwitcher() {
    ...
    var theme = saved || 'theme-blue'; // 默认商务蓝
    applyTheme(theme);
}
